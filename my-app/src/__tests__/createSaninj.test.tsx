...
import payloads from './payloadsinj.json';
import fs from 'fs';

describe('Blog post Sanitize injection test', () => {
  ...
  it.each(payloadArray)('Testing with payload %s from category %s', async (payload: string, category: string) => {
      await page.goto('http://localhost:3000/createsan');
  ...
  afterAll(()=>{
    ...
    console.info(results);
     fs.writeFileSync('../resultssan/resultsinj.json', JSON.stringify(results));
  })
